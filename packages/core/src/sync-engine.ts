import { ConflictResolver, type ConflictResolution } from './conflict.js';
...
export class SyncEngine {
  ...
  private eventHistory: SyncEventRecord[] = [];
  ...
  reconcile(
    ...
    return this.runExclusive(async () => {
      ...
      try {
        ...
        if (options.cursor !== undefined) this.meta.cursor = options.cursor;
  ...
  reconcileSnapshot(
    ...
    if (options.deleteMissing) {
      for (const local of this.options.store.list(entity)) {
        if (options.includeLocal && !options.includeLocal(local)) continue;
  ...
  inspectEvents(limit = 100): SyncEventRecord[] {
  ...
  private async pullInternal(): Promise<PullResult | null> {
    if (!this.options.transport.pull) return null;
  ...
  private applyDelta(delta: RemoteDelta): void {
    ...
    const remote = delta.op === 'delete'
      ...
      : delta.op === 'patch'
        ...
        : delta.data;
    const pendingMutations = this.options.queue.all().filter((record) => (
      record.effects.some((effect) => effect.entity === delta.entity && effect.id === delta.id)
    ));
  ...
  private refreshStatus(patch: Partial<SyncStatus> = {}): void {
    this.currentStatus = {
      ...this.currentStatus,
